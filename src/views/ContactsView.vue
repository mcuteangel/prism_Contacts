<template>
  <div class="p-4 max-w-2xl mx-auto">
    <div v-if="!userStore.currentUser" class="glass p-6 max-w-md mx-auto mt-10">
      <h2 class="text-xl font-bold mb-4 text-center">{{ showRegister ? 'ثبت‌نام' : 'ورود' }}</h2>
      <form v-if="!showRegister" @submit.prevent="handleLogin" class="flex flex-col gap-3">
        <input v-model="loginForm.username" type="text" placeholder="نام کاربری" class="input" required />
        <input v-model="loginForm.password" type="password" placeholder="رمز عبور" class="input" required />
        <button type="submit" class="btn btn-primary">ورود</button>
        <div class="text-xs text-center mt-2">
          حساب ندارید؟ <button type="button" class="text-blue-600 underline" @click="showRegister = true">ثبت‌نام</button>
        </div>
        <div v-if="userStore.error" class="text-red-500 text-xs text-center">{{ userStore.error }}</div>
      </form>
      <form v-else @submit.prevent="handleRegister" class="flex flex-col gap-3">
        <input v-model="registerForm.username" type="text" placeholder="نام کاربری" class="input" required />
        <input v-model="registerForm.password" type="password" placeholder="رمز عبور" class="input" required />
        <button type="submit" class="btn btn-success">ثبت‌نام</button>
        <div class="text-xs text-center mt-2">
          حساب دارید؟ <button type="button" class="text-blue-600 underline" @click="showRegister = false">ورود</button>
        </div>
        <div v-if="userStore.error" class="text-red-500 text-xs text-center">{{ userStore.error }}</div>
      </form>
    </div>
    <template v-else>
      <div class="flex justify-between items-center mb-2 gap-2">
        <div class="text-xs text-gray-600">کاربر: {{ userStore.currentUser.username }} ({{ userStore.currentUser.role === 'admin' ? 'مدیر' : 'عادی' }})</div>
        <div class="flex gap-2">
          <button @click="toggleTheme" class="btn btn-secondary">{{ isDark ? 'تم روشن' : 'تم تاریک' }}</button>
          <button @click="handleLogout" class="btn btn-error">خروج</button>
        </div>
      </div>
      ...
      <div class="flex flex-col sm:flex-row gap-2 mb-4 justify-center">
        <button @click="exportContacts" class="btn btn-secondary flex-1">خروجی JSON</button>
        <button @click="exportContactsCsv" class="btn btn-secondary flex-1">خروجی CSV</button>
        <label class="btn btn-secondary flex-1 cursor-pointer">
          ورود از JSON
          <input type="file" accept="application/json" class="hidden" @change="importContacts" />
        </label>
        <label class="btn btn-secondary flex-1 cursor-pointer">
          ورود از CSV
          <input type="file" accept="text/csv" class="hidden" @change="importContactsCsv" />
        </label>
      </div>
      <ContactForm :form="form" :editingId="editingId" :groups="groups.map(g => ({ id: g.id ?? 0, name: g.name }))" @submit="onFormSubmit" @cancel="cancelEdit" />
      <div class="flex flex-col sm:flex-row gap-2 mb-4">
        <input v-model="search" type="text" placeholder="جستجو بر اساس نام یا شماره..." class="input flex-1" />
      ...
      <div v-if="filteredContacts.length === 0" class="text-gray-500 text-center">مخاطبی وجود ندارد.</div>
      <ContactList :contacts="filteredContacts" :groups="groups.map(g => ({ id: g.id ?? 0, name: g.name }))" @edit="edit" @remove="remove" />
      <div v-if="showDuplicates" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
        <div class="glass p-6 max-w-lg w-full relative">
          <button @click="showDuplicates = false" class="absolute top-2 left-2 btn btn-error btn-xs">بستن</button>
          <h3 class="font-bold mb-2">مخاطبین تکراری</h3>
          <div v-if="duplicates.length === 0" class="text-gray-500">مخاطب تکراری یافت نشد.</div>
          <ul v-else>
            <li v-for="group in duplicates" :key="group.key" class="mb-4">
              <div class="font-bold text-blue-700 mb-1 flex items-center gap-2">
                {{ group.key }}
                <button @click="mergeGroup(group.contacts)" class="btn btn-success btn-xs ml-2">ادغام همه</button>
              </div>
              <ul class="ml-4">
                <li v-for="c in group.contacts" :key="c.id" class="flex items-center gap-2">
                  {{ c.name }} - {{ c.phoneNumbers[0] }}
                  <button @click="remove(c.id)" class="btn btn-error btn-xs">حذف</button>
                </li>
              </ul>
            </li>
          </ul>
          ...
          <div v-if="successMsg" class="fixed top-4 left-1/2 -translate-x-1/2 bg-green-500 text-white px-4 py-2 rounded shadow z-50">
            {{ successMsg }}
          </div>
        </div>
      </div>
...
const add = async (data: { name: string; phone: string; groupIds?: number[] }) => {
...
const update = async (data: { name: string; phone: string; groupIds?: number[] }) => {
...
const onFormSubmit = (data: { name: string; phone: string; groupIds?: number[] }) => {
...
const edit = (c: any) => {
  ...
  form.value = { name: c.name, phone: c.phoneNumbers[0], groupIds: c.groupIds ? [...c.groupIds] : [] }
}
...
const exportContactsCsv = () => {
  ...
  const customKeys = Array.from(new Set(contacts.flatMap(c => c.customFields ? Object.keys(c.customFields) : [])))
  const headers = [
    'name',
    'phoneNumbers',
    'groupNames',
  ...
  const rows = contacts.map(c => {
    const groupNames = c.groupIds && groups.length
...
const importContactsCsv = async (e: Event) => {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (obj.groupNames && groups.length) {
      ...
      contact.groupIds = groupNames.map((gn: string) => {
...
const filteredContacts = computed(() => {
  ...
  if (search.value) {
    ...
    list = list.filter(c =>
      ...
      (c.customFields && Object.values(c.customFields).some(val => String(val).toLowerCase().includes(q)))
    )
  }
  list = [...list].sort((a, b) => {
...
const fuzzyDuplicates = computed(() => {
  ...
  for (let i = 0; i < contacts.length; i++) {
    ...
    for (let j = i + 1; j < contacts.length; j++) {
      ...
      const phoneSim = contacts[i].phoneNumbers[0] && contacts[j].phoneNumbers[0]
        ? stringSimilarity.compareTwoStrings(contacts[i].phoneNumbers[0], contacts[j].phoneNumbers[0])
        : 0
    ...
    if (group.length > 1) {
      ...
      group.forEach(c => { if (typeof c.id === 'number') used.add(c.id) })
    }
  }
  return result
})
...
const mergeGroup = async (contactsToMerge: any[]) => {
  ...
  for (const c of rest) {
    // Merge phone numbers
    merged.phoneNumbers = Array.from(new Set([...(merged.phoneNumbers || []), ...(c.phoneNumbers || [])]))
    // Merge groupIds
